...
void SpotMicroKinematics::setFeetPosGlobalCoordinates(
  ...
  right_front_leg_.setFootPosGlobalCoordinates(four_legs_foot_pos.right_front,
                                               ht_rf);

  left_front_leg_.setFootPosGlobalCoordinates(four_legs_foot_pos.left_front,
                                              ht_lf);

  left_back_leg_.setFootPosGlobalCoordinates(four_legs_foot_pos.left_back,
                                             ht_lb);
}
...
BodyState SpotMicroKinematics::getBodyState() {
  BodyState body_state = {EulerAngs{phi_, theta_, psi_}, Point{x_, y_, z_},
                          getLegsFootPos()};
  return body_state;
}
...
AllRobotRelativeTransforms SpotMicroKinematics::getRobotTransforms() {
  ...
  allTransforms.centerToRightBack =
      htLegRightBack(smc_.body_length, smc_.body_width);

  allTransforms.centerToRightFront =
      htLegRightFront(smc_.body_length, smc_.body_width);

  allTransforms.centerToLeftFront =
      htLegLeftFront(smc_.body_length, smc_.body_width);

  allTransforms.centerToLeftBack =
      htLegLeftBack(smc_.body_length, smc_.body_width);
