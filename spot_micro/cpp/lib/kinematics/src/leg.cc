...
SpotMicroLeg::SpotMicroLeg(const JointAngles &joint_angles,
                           const LinkLengths &link_lengths, bool is_leg_12)
...
Point SpotMicroLeg::getFootPosGlobalCoordinates(const Matrix4f &ht_leg_start) {
  ...
  Matrix4f ht_foot = ht_leg_start * ht0To4(joint_angles_, link_lengths_);

  // Construct return point structure
  Point return_point = {
      .x = ht_foot(0, 3), .y = ht_foot(1, 3), .z = ht_foot(2, 3)};
  return return_point;
}
...
Matrix4f SpotMicroLeg::getTransform1To3() {
  return (ht1To2() * ht2To3(joint_angles_.ang2, link_lengths_.l2));
}
